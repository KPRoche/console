...
func NewServer(cfg Config) (*Server, error) {
	...
	// tokenExplicit: true when the token was provided by an operator (not auto-generated).
	// When KC_AGENT_TOKEN_GENERATED=true (set by startup scripts), the token is treated
	// as auto-generated even if KC_AGENT_TOKEN is set, enabling origin-based auth bypass
	// for browser clients (#12587, #12444).
	tokenExplicit := agentToken != "" && os.Getenv("KC_AGENT_TOKEN_GENERATED") != "true"
	if tokenExplicit {
		slog.Info("Agent token authentication enabled (from KC_AGENT_TOKEN)")
	} else if agentToken != "" {
		slog.Info("Agent token authentication enabled (auto-generated, origin-based bypass allowed for browsers)")
	} else {
