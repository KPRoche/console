...
// GetDevToken returns a JWT token for dev mode without requiring OAuth.
// Only works when devMode is enabled and no OAuth client is configured.
// Used by Quantum cards and other components to fetch live data in dev/demo mode.
func (h *AuthHandler) GetDevToken(c *fiber.Ctx) error {
	// Only allow in dev mode with no OAuth configured
	if !h.devMode || h.oauthConfig.ClientID != "" {
		return fiber.NewError(fiber.StatusForbidden, "Dev token endpoint disabled")
	}

	// Get or create dev user (same logic as devModeLogin)
	var devLogin, devEmail, avatarURL, devGitHubID string
	...
	// Return token in response (for components to use if needed)
	return c.JSON(fiber.Map{
		"token":     jwtToken,
		"user_id":   user.ID,
